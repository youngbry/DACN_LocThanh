...
async function main() {
  console.log('Deploying contracts to network:', hre?.network?.name || 'localhost');
  ...
  const MotorbikeMarketplace = await ethers.getContractFactory("MotorbikeMarketplace");
  ...
  const marketplaceAbi = JSON.parse(MotorbikeMarketplace.interface.formatJson());

  // Write to scripts folder (for record)
  fs.writeFileSync(path.join(__dirname, 'MotorbikeNFT-abi.json'), JSON.stringify(nftAbi, null, 2));
  fs.writeFileSync(path.join(__dirname, 'MotorbikeNFT-address.txt'), nftAddress);
  fs.writeFileSync(path.join(__dirname, 'MotorbikeMarketplace-abi.json'), JSON.stringify(marketplaceAbi, null, 2));
  fs.writeFileSync(path.join(__dirname, 'MotorbikeMarketplace-address.txt'), marketplaceAddress);
