...
const ListNFT = () => {
  ...
  const loadNFTInfo = async () => {
    try {
      ...
      if (typeof window.ethereum !== "undefined") {
        ...
        setIsApproved(
          approvedAddress === MARKETPLACE_ADDRESS || isApprovedForAll
        );
      }
    } catch (error) {
  ...
  const listNFT = async () => {
    ...
    // Pre-check locked state before listing
    try {
      const provider = new ethers.BrowserProvider(window.ethereum);
      const nftContract = new ethers.Contract(
        CONTRACT_ADDRESS,
      ...
      const isLocked = await nftContract.locked(tokenId);
      if (isLocked) {
        alert("❌ NFT đang bị khóa. Không thể đăng bán.");
        return;
      }
    } catch (lockErr) {
      console.warn(
        "Không kiểm tra được trạng thái khóa trước khi list:",
        lockErr
      );
    }
    ...
    try {
      ...
      if (error.message.includes("user rejected")) {
        ...
        errorMessage = "NFT đang bị khóa và không thể đăng bán";
      }
  ...
  return (
    <div className="list-nft">
      <div className="list-nft-header">
        ...
        <div className="info-badge">💡 NFT sẽ hiển thị trên marketplace</div>
      </div>
