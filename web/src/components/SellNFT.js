...
const SellNFT = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.buyerAddress.trim()) {
      ...
      formData.buyerAddress.toLowerCase() === userAddress.toLowerCase()
    ) {
  ...
  const handleTransfer = async (e) => {
    ...
    // Pre-check locked state before transfer
    try {
      const provider = new ethers.BrowserProvider(window.ethereum);
      ...
      const isLocked = await nftRead.locked(tokenId);
      if (isLocked) {
        alert("❌ NFT đang bị khóa. Không thể chuyển nhượng.");
        return;
      }
    } catch (lockErr) {
      console.warn(
        "Không kiểm tra được trạng thái khóa trước khi chuyển:",
        lockErr
      );
    }
    ...
    try {
      ...
      if (error.message.includes("user rejected")) {
        ...
        error.message.includes("Token is locked") ||
        error.message.includes("Token locked")
      ) {
        errorMessage = "NFT đang bị khóa. Không thể chuyển nhượng";
      }
  ...
  if (!nft || !isOwner) {
    return (
      <div className="sell-nft-error">
        ...
        <h2>Không thể chuyển nhượng NFT</h2>
  ...
  return (
    <div className="sell-nft">
      <div className="sell-nft-header">
        <h1>💸 Chuyển nhượng NFT #{tokenId}</h1>
        <div className="warning-badge">⚠️ Thao tác không thể hoàn tác</div>
      </div>
        ...
        <div className="transfer-form-section">
          ...
          <form onSubmit={handleTransfer} className="transfer-form">
            <div className="form-group">
              <label htmlFor="buyerAddress">Địa chỉ ví người mua *</label>
              <input
                type="text"
                id="buyerAddress"
                name="buyerAddress"
            ...
            <div className="form-group">
              <label htmlFor="confirmAddress">Xác nhận địa chỉ ví *</label>
              <input
                type="text"
                id="confirmAddress"
                name="confirmAddress"
      ...
      {/* Footer back button removed; cancel now handles back navigation */}
    </div>
  );
};
